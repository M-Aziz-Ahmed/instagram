...
router.get("/smart", async (req, res) => {
    try {
        ...
        if (!viewer) {
            ...
            return res.json({ posts: await enrichPosts(posts), hasMore: posts.length === limit });
        }
        ...
        const expiryCondition = { $or: [{ expiresAt: null }, { expiresAt: { $gt: new Date() } }] };
        ...
        return res.json({ posts: await enrichPosts(sliced), hasMore });
    } catch (err) {
