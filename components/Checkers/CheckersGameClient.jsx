...
export default function CheckersGameClient({ gameId }) {
    const { user } = useUser();
    const { creating, playAgain } = useGameReplay("checkers");
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await fetch(`/api/checkers/games/${gameId}`);
                if (res.ok) {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
                    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-gray-200 dark:border-gray-700 text-center">
                        ...
                        <div className="flex flex-col gap-2">
                            <button onClick={() => playAgain("/api/checkers/games", { mode: game.mode, aiDifficulty: game.aiDifficulty })} disabled={creating} className="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl text-sm">{creating ? "Starting..." : "Play Again"}</button>
