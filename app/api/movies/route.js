...
const TMDB_API_KEY = process.env.TMDB_API_KEY || "demo_key_for_testing"; // Use environment variable in production
const TMDB_BASE_URL = "https://api.themoviedb.org/3";export async function GET(request) {
  const { searchParams } = new URL(request.url);
  const query = searchParams.get("q");
  const id = searchParams.get("id");

  if (query) {
    const searchUrl = `${TMDB_BASE_URL}/search/movie?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&language=en-US&page=1`;
  ...
  if (id) {
    const detailsUrl = `${TMDB_BASE_URL}/movie/${id}?api_key=${TMDB_API_KEY}&language=en-US&append_to_response=credits,videos`;
  ...
  if (searchParams.get("trending")) {
    ...
    const trendingUrl = `${TMDB_BASE_URL}/trending/movie/${timeWindow}?api_key=${TMDB_API_KEY}&language=en-US`;
  ...
  const upcomingUrl = `${TMDB_BASE_URL}/movie/upcoming?api_key=${TMDB_API_KEY}&language=en-US&page=${page}`;
